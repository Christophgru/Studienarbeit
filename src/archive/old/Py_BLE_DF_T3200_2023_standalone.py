...
#### Global Variables #### 
#TODO auskommentieren, für Hardware-Testing
""" serialdata_anchor1 = serial.Serial('COM3', 115200, timeout=0.05, rtscts=1)  # baudrate = 115200, ready to send/ clear to send = 1, If a timeout is set it may return less characters as requested. With no timeout it will block until the requested number of bytes is read.
serialdata_anchor2 = serial.Serial('COM4', 115200, timeout=0.05, rtscts=1) """
# define a FIFO Queue for each anchor to exchange records safely between multiple threads
#fifo_anchor1: Queue[Any] = queue.Queue() # Constructor for FIFO queue (defined size is infinite) 
#fifo_anchor2: Queue[Any] = queue.Queue()
    ...
    try:
        ...
        # Sender liegt rechtsseitig des hinteren Empfängers
            ...
            if theta2-getDeviation(theta2+theta2offset) > theta3+getDeviation(theta3+theta3offset):
                [xmax, ymax] = calcXY(theta2-getDeviation(theta2+theta2offset),theta3+getDeviation(theta3+theta3offset),distance)
        ...
        # Sender liegt linksseitig des vorderen Empfängers
            ...
            if theta2+getDeviation(theta2+theta2offset) < theta3-getDeviation(theta3+theta3offset):
                [unused, ymax_x] = calcXY(theta2+getDeviation(theta2+theta2offset),theta3-getDeviation(theta3+theta3offset),distance)
                yinaccuracyx = abs(ymax_x-ywahr)
                [unused, ymax_y] = calcXY(theta2-getDeviation(theta2+theta2offset),theta3+getDeviation(theta3+theta3offset),distance)
...
def main():
    ...
    try:
        ...
        plt.plot(*anchor1_coordinates, marker="o", markersize=10, markeredgecolor="blue", markerfacecolor="blue", label="Anchor 1")
        plt.plot(*anchor2_coordinates, marker="o", markersize=10, markeredgecolor="blue", markerfacecolor="blue", label="Anchor 2")

        tag_point = plt.plot(1, 1, marker="o", markersize=10, markeredgecolor="black", markerfacecolor="black", label="helmet")[0]
        ...
        while 1:
            ...
            if serialdata_anchor1.in_waiting > 0: # check wether there are data on COM Port for anchor node 1 
                dataStream_anchor1 = str(serialdata_anchor1.read(80)) # cast 80 bytes to string (approximated lenght of +UUDF event is 60 bytes, additionally there are empty lines between two events, 80 proved to be favourable)
                regex_anchor1 = re.split("UUDF:", dataStream_anchor1) # regular expression split the data on the UUDF: string for work on events
                for listing in regex_anchor1:
                        if "6C1DEBA0982F" in listing: # check if the event data is corresponding to the correct anchor node
                            ...
                            if len(parts) == 9: # check if the splitted event data is complete
                                fifo_anchor1.put(int(parts[2])+theta2_offset) # fetches the azimut data (theta2) from the string and add the rotational offset value 
            ...
            if fifo_anchor1.qsize() >= 4 and fifo_anchor2.qsize() >= 4: # check the queue size 
                ...
                for i in range(4):
                    records_anchor1.append(fifo_anchor1.get()) # the get function transfers and then erases the first (oldest) listing in the queue
                ...
                max_deviation_listing_anchor1 = max(records_anchor1, key=lambda x: abs(x - arithmetic_mean_fifo_anchor1))
                ...
                max_deviation_listing2 = max(records_anchor2, key=lambda x: abs(x - arithmetic_mean_fifo_anchor2_listing))
                ...
                # inaccuracy = get_inaccuracy(theta2*180/pi,theta3*180/pi,distance,theta2_offset,theta3_offset)
                # # indicate the risk of an accident
                # if inaccuracy < 1:
                #     tag_point.set_markerfacecolor('green')
                # elif inaccuracy < 2:
                #     tag_point.set_markerfacecolor('yellow')
                # else:
                #     tag_point.set_markerfacecolor('red')
